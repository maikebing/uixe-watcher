...
namespace Vnc.Viewer
{
    ...
    public class Conn : IDisposable
    {
        ...
        public bool IsFmtChgPending = false;
        ...
        private void Connect()
        {
            ...
            {
                throw new WarnEx("无法连接至" + opts.Host + ".");
            }
        }
        ...
        public void WriteBytes(byte[] bytes, RfbCliMsgType msgType)
        {
            WriteBytes(bytes);
        }
        ...
        public void Run(ViewOpts viewOpts)
        {
            this.viewOpts = viewOpts;
            Run();
        }

        public void Run(ConnOpts opts)
        {
            this.opts = opts;
            Run();
        }
        ...
        public void Run()
        {
            ...
            catch (WarnEx ex)
            {
                this.parentForm.Invoke((MethodInvoker)delegate
            {
                XtraMessageBox.Show(this.opts.Host + "\r\n" + ex.Message,
                                App.GetStr("错误"),
                                MessageBoxButtons.OK,
                                MessageBoxIcon.Exclamation,
                                MessageBoxDefaultButton.Button1);
        ...
        public void SendSetScale(byte scale)
        {
        ...
        public void SendUpdReq(bool incremental)
        {
            if (IsFmtChgPending)
            {
        ...
        public void Dispose()
        {
            if (view != null && view.IsHandleCreated && !view.IsDisposed)
            {
                view.Dispose();
            }
        }
