...
namespace Vnc.Viewer
{
    public abstract class View : Control
    {
        ...
        public void CopyRect(Rectangle rect, UInt16 x, UInt16 y)
        {
        ...
        public void ResizeFrameBuf(UInt16 width, UInt16 height)
        {
            newWidth = width;
            newHeight = height;
            isFBResized = true;
        ...
        public void NewFrameBuf(UInt16 width, UInt16 height)
        {
            newWidth = width;
            newHeight = height;
            isFBResized = false;
        ...
        public void LockFrameBuf()
        {
            Monitor.Enter(this);
        }

        public void UnlockFrameBuf()
        {
            Monitor.Exit(this);
        }
        ...
        private void ResizeCore()
        {
            ...
            {
                double b = (double)rawFBWidth / (double)rawFBHeight;
                if (this.Width < 800 || this.Height < 600)
                {
                    int h = this.Height - this.ClientSize.Height;
                    int w = this.Width - this.ClientSize.Width;
                    this.Width = (int)(rawFBWidth * 0.75) + w;
                    this.Height = (int)(rawFBHeight * 0.75) + h;
                }
                if (ClientSize.Height * b <= ClientSize.Width)
                {
                    scaledFBWidth = (UInt16)(ClientSize.Height * b);
                    scaledFBHeight = (UInt16)ClientSize.Height;
                }
                else
                {
                    scaledFBWidth = (UInt16)ClientSize.Width;
                    scaledFBHeight = (UInt16)(ClientSize.Width / b);
                }
                //scaledFBWidth = (UInt16)ClientSize.Width;
                //scaledFBHeight = (UInt16)ClientSize.Height;
            }
