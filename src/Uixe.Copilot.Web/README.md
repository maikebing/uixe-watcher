...
> 前端工具链当前要求 **Node.js LTS 22.12.0 或更高版本**。如果本机仍是 Node 18/20 的旧版本，`vite@7` 构建会失败。
...
当前目录已新增 `Dockerfile`，默认也使用 Node LTS 22。

构建后即可在容器内运行 Vite 开发服务，避免宿主机 Node 版本过旧导致的构建失败。
